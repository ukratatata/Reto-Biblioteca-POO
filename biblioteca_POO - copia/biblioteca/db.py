...
class BibliotecaRepository:
    ...
    def _siguiente_id(self, tabla: str, columna_pk: str, prefijo: str) -> str:
        """
        Genera el siguiente ID correlativo con el prefijo indicado.
        Busca el número más alto existente y suma 1, garantizando que no haya colisión.
        Formato resultante: prefijo + número de 4 dígitos, ej: US-0016, MFL-0003.
        """
        conexion = self._conectar()
        cursor = conexion.cursor()

        # Traemos todos los IDs que empiecen por el prefijo y buscamos el máximo numérico
        cursor.execute(
            f"SELECT {columna_pk} FROM {tabla} WHERE {columna_pk} LIKE ?",
            (f"{prefijo}%",)
        )
        filas = cursor.fetchall()
        conexion.close()
        ...
        maximo = 0
        for (id_existente,) in filas:
            try:
                numero = int(id_existente.replace(prefijo, ""))
                if numero > maximo:
                    maximo = numero
            except ValueError:
                pass    # ID con formato distinto; lo ignoramos

        return f"{prefijo}{maximo + 1:04d}"

    def siguiente_id_socio(self) -> str:
        """Devuelve el siguiente ID libre para un Socio. Ej: US-0016"""
        return self._siguiente_id("usuarios", "id_usuario", "US-")

    def siguiente_id_empleado(self) -> str:
        """Devuelve el siguiente ID libre para un Empleado. Ej: UE-0006"""
        return self._siguiente_id("usuarios", "id_usuario", "UE-")

    def siguiente_id_libro(self) -> str:
        """Devuelve el siguiente ID libre para un Libro. Ej: MFL-0011"""
        return self._siguiente_id("materiales", "codigo_id", "MFL-")

    def siguiente_id_revista(self) -> str:
        """Devuelve el siguiente ID libre para una Revista. Ej: MFR-0006"""
        return self._siguiente_id("materiales", "codigo_id", "MFR-")

    def siguiente_id_dispositivo(self) -> str:
        """Devuelve el siguiente ID libre para un Dispositivo. Ej: MFD-0006"""
        return self._siguiente_id("materiales", "codigo_id", "MFD-")

    def siguiente_id_juego(self) -> str:
        """Devuelve el siguiente ID libre para un JuegoDeMesa. Ej: MFJ-0006"""
        return self._siguiente_id("materiales", "codigo_id", "MFJ-")

    def siguiente_id_digital(self) -> str:
        """Devuelve el siguiente ID libre para un RecursoDigital. Ej: MFG-0006"""
        return self._siguiente_id("materiales", "codigo_id", "MFG-")
